...
void PlanManager::process(const ros::TimerEvent &)
{
...

    {
        ...
        std::vector<int> refined_singuals;
        refined_singuals.resize(segnum);
        ...
        refined_gearPos.resize(segnum - 1);
        refined_angles.resize(segnum - 1);
        double basetime = 0.0;
        ...
        // Qilinn: 被我注释掉了,如下是一个经验时间除以每段轨迹的分段数获得每个小分段的时间
        ...
        {
            //---------- 检测输入IN ------------//
            ROS_WARN("Qilinn: Before ARC-opt");
            ROS_INFO_STREAM("iniState2d:\n " << iniState2d);
            ROS_INFO_STREAM("finState2d:\n " << finState2d);
            ROS_INFO_STREAM("refined_rt:\n " << refined_rt);
            ROS_INFO("refined_inPs_container: ");
            for (int i = 0; i < refined_inPs_container.size(); ++i)
            {
                ROS_INFO_STREAM("Segment " << i << ":\n"
                                           << refined_inPs_container[i]);
            }
            ROS_INFO_STREAM("refined_gearPos: ");
            for (int i = 0; i < refined_gearPos.size(); ++i)
            {
                ROS_INFO_STREAM("Segment " << i << ": " << refined_gearPos[i].transpose());
            }
            ROS_INFO_STREAM("refined_angles: ");
            for (int i = 0; i < refined_angles.size(); ++i)
            {
                ROS_INFO_STREAM("Segment " << i << ": " << refined_angles[i]);
            }
            ROS_INFO_STREAM("refined_singuals: ");
            for (int i = 0; i < refined_singuals.size(); ++i)
            {
                ROS_INFO_STREAM("Segment " << i << ": " << refined_singuals[i]);
            }
            //---------- 检测输入OUT ------------//
            PolyTrajOpt::TrajOpt refinedOpt;
            // 25ms
            ...
            PolyTrajOpt::DpTrajectory FirstDptraj = optTraj.Traj_container[0];
            for (double t = 0.0; t <= FirstDptraj.getTotalDuration(); t += 0.01)
            {
                Eigen::Vector2d pos = FirstDptraj.getPos(t);
                Eigen::Vector2d vel = FirstDptraj.getVel(t);
                Eigen::Vector2d acc = FirstDptraj.getAcc(t);
                std::cout << "Time: " << t << " Vel: " << vel.transpose() << " Acc: " << acc.transpose() << std::endl;
            }
