...
namespace PolyTrajOpt
{
    ...
    public:
            ...
            std::shared_ptr<visualization::Visualization> vis_tool, std::string name = "nn", std::vector<Eigen::VectorXd> ds_container = {})
        {
                ...
                {
                    {
                        ...
                        Eigen::MatrixXd tmpPts;
                        tmpPts.resize(2, piecenums[i] + 1);
                        tmpPts.col(0) << tmpHead;
                        tmpPts.col(piecenums[i]) << tmpTail;
            ...
            // Print key input arguments
            std::cout << "OptimizeSe2Trajectory key inputs:" << std::endl;
            std::cout << "name: " << name_ << std::endl;
            std::cout << "headState2d_:\n"
                      << headState2d_ << std::endl;
            std::cout << "tailState2d_:\n"
                      << tailState2d_ << std::endl;
            std::cout << "rdTs_container: " << rdTs_container.transpose() << std::endl;

            std::cout << "singual (" << singual_.size() << "): ";
            for (size_t i = 0; i < singual_.size(); ++i)
            {
                std::cout << singual_[i] << (i + 1 == singual_.size() ? "" : " ");
            }
            std::cout << std::endl;

            std::cout << "gearPos (" << gearPos.size() << "): ";
            for (size_t i = 0; i < gearPos.size(); ++i)
            {
                std::cout << "[" << gearPos[i].x() << ", " << gearPos[i].y() << "]";
                if (i + 1 != gearPos.size())
                    std::cout << ", ";
            }
            std::cout << std::endl;
            ...
            {
                std::cout << angles[i] << (i + 1 == angles.size() ? "" : ", ");
            }
            std::cout << std::endl;

            std::cout << "inPs_container(innerPts): ";
            for (size_t i = 0; i < inPs_container.size(); ++i)
            {
                std::cout << inPs_container[i].rows() << "x" << inPs_container[i].cols();
                if (i + 1 != inPs_container.size())
                    std::cout << ", ";
                std::cout << inPs_container[i].transpose() << "; ";
            }
            std::cout << std::endl;

            // if (!ds_container.empty()) {
            //     std::cout << "ds_container sizes: ";
            //     for (size_t i = 0; i < ds_container.size(); ++i) {
            //         std::cout << ds_container[i].size();
            //         if (i + 1 != ds_container.size()) std::cout << ", ";
            //     }
            //     std::cout << std::endl;
            // } else {
            //     std::cout << "ds_container: <empty>" << std::endl;
            // }
        ...
        static inline double PenaltyCostFunctionCallback(void *func_data, const Eigen::VectorXd &x, Eigen::VectorXd &grad)
        {
            ...
            Eigen::VectorXd gradDts(opt->trajnum);
            gradDts.setZero();
            ...
            for (int trajid = 0; trajid < opt->trajnum; trajid++)
            {
                ...
                Eigen::VectorXd gradRs(opt->piecenums[trajid]);
                gradRs.setZero();
            ...
            for (int trajid = 0; trajid < opt->trajnum; trajid++)
            {
...

                {
                    ...
                    wpTs.resize(1, opt->piecenums[trajid] - 1);
                    IniSt.resize(1, 3);
                    FinSt.resize(1, 3);
        ...
        void penaltyGradCost2CT(double &cost, const PolyTrajOpt::MINCO_S3NU<2> &posOpt, const PolyTrajOpt::MINCO_S3NU<1> &tOpt, Eigen::MatrixXd &gdC2c_2d, Eigen::VectorXd &gdC2t_2d, Eigen::MatrixXd &gdC2c_1d, Eigen::VectorXd &gdC2t_1d, int trajid)
        {
            ...
            for (int i = 0; i < N; ++i)
            {
                ...
                for (int j = 0; j <= traj_res; ++j)
                {
                    ...
                    // if (isFirstTimeOpt_) // qilinn-debug: 输出参数化后的轨迹点
                    // {
                    ...
                    /*curvature constraint penalty*/
                    {
                        ...
                        if (vioCur > 0.0)
                        {
                            ...
                            gradViolaCv = (2 * cross * Eigen::Vector2d(acc[1], -acc[0]) / (vel.squaredNorm() * vel.squaredNorm() * vel.squaredNorm()) -
                            ...
                            */
                            ...
                            if (vioSdist > 0)
                            {
                                ...
                                gradVel += esdfWei * penaD * (-1.0) *
                                           (sg * temp_l_Bl / vel.norm() - vel * (rotR * resPt).transpose() / (vel.squaredNorm())) * gradViolaSdpos;
                            }
                        }
                    }
